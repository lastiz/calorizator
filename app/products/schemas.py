...
class ProductScheme(BaseModel):
    ...
    ingredients: list[ProductIngredientScheme] = Field(
        min_length=1, description="Product ingredients in grams"
    )
...
class AddProductScheme(BaseModel):
    ...
    title: str = Field(min_length=3, max_length=320, description="Product Title")
    ingredients: list[ProductIngredientScheme] = Field(
        min_length=1, description="Product ingredients"
    )
...
class UpdateProductScheme(BaseModel):
    ...
    ingredients: list[ProductIngredientScheme] | None = Field(
        min_length=1, description="Product ingredients"
    )
