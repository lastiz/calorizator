...
class User(Model, TimeMixin):
    ...
    token = fields.CharField(
        max_length=516, indexable=True, null=True, blank=True, unique=True
    )
...
class ProductIngredient(Model):
    ...
    product = fields.ForeignKeyField(
        "models.Product", on_delete=OnDelete.CASCADE, related_name="ingredients"
    )
    ingredient = fields.ForeignKeyField(
        "models.Ingredient", on_delete=OnDelete.CASCADE, related_name="products"
    )
